            ...
            <div className="hidden md:flex items-center gap-8">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="relative text-sm font-medium text-[#A3A3A3] hover:text-[#FAFAFA] transition-colors duration-300 group"
                >
                  {item.name}
        ...
        {isMobileMenuOpen && (
          ...
          >
            ...
            <motion.div
              ...
              className="absolute right-0 top-0 bottom-0 w-72 bg-[#141414] border-l border-[#C4956A]/20 pt-24 px-6"
            >
                ...
                {navItems.map((item, index) => (
                  ...
                  >
                    <Link
                      href={item.href}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="text-lg font-medium text-[#FAFAFA] hover:text-[#C4956A] transition-colors"
                    >
                      {item.name}
                    </Link>
                  </motion.div>
